...
export async function SkateboardProduct({ id }: Props) {
  ...
  return (
    <div className="group relative mx-auto w-full max-w-72 px-8 pt-4">
      ...
      <h3 className="my-2 text-center font-sans leading-tight text-lg lg:text-xl">
        {product.data.name}
      </h3>

      <div className="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity duration-200 group-hover:opacity-100">
        <ButtonLink size="sm" color="orange" field={product.data.customizer_link}>Customize</ButtonLink>
      </div>
    </div>
  );
}
